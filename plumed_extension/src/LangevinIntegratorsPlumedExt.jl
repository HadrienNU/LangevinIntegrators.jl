module LangevinIntegratorsPlumedExt
using CBinding
...
function plumed_init!(plumed_struct; kwargs...)
    ...
    if plumed_api_version < 6 || plumed_api_version > 11  # TODO: Faire des Tests sur l'api de 6 à 9
        println(
            "Incompatible API version $plumed_api_version for PLUMED. Only Plumed 2.5.x, 2.6.x, and 2.7.x are tested and supported.",
        )
    end
...
function plumed_step!(plumed_struct::plumed, n, positions, forces, energy)
    ...
    c"plumed_cmd"(
        plumed_struct.plumedmain,
        "setPositions",
        Ref(plumed_struct.position_plumed, 1),
    )
    c"plumed_cmd"(
        plumed_struct.plumedmain,
        "setForces",
        Ref(plumed_struct.forces_plumed, 1),
    )                 # Pass a pointer to the first element in the foces array to plumed
    ...
    c"plumed_cmd"(
        plumed_struct.plumedmain,
        "isEnergyNeeded",
        plumed_struct.ref_needs_energy,
    )# assuming flag is an int, that will be set to 0 if energy is not needed and 1 if it is needed
