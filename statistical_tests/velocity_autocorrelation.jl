...
let
    ...
    params = TrajsParams(n_steps = 5 * 10^5, n_trajs = 20, n_save_iters = 1)
    Δt = 1e-3
    factor_length = 10
    tps = LinRange(
        0,
    ...
    plot(
        tps,
        exp.(-0.5 * γ * tps) .* (cos.(ω * tps) - 0.5 * γ / ω * sin.(ω * tps)) / β,
        label = "Theory",
    )
    for int_class in [ABOBA, BAOAB, BBK, GJF, VEC, OBABO]
        ...
        for trj in trajs
            corr .+=
                autocov(trj.xt[1][10^3:end], 0:div(params.n_steps, factor_length))[:, 1] /
                params.n_trajs
        end
