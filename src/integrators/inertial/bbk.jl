...
function UpdateState!(state::VelocityVerletState, integrator::BBK; kwargs...)
    ...
    state.v_mid = state.v .+ 0.5 * integrator.Δt / integrator.M * state.f .- 0.5 * integrator.Δt .* integrator.γ * state.v .+ 0.5*state.ξ
    @. state.x = state.x + integrator.Δt * state.v_mid
    apply_space!(integrator.bc,state.x,state.v)
    nostop = forceUpdate!(integrator.force, state.f, state.x; kwargs...)
    state.ξ = integrator.σ * randn(integrator.dim)
    state.v = (state.v_mid .+ 0.5 * integrator.Δt / integrator.M * state.f + 0.5*state.ξ) / (1 + 0.5 * integrator.Δt * integrator.γ)
    return nostop
end
