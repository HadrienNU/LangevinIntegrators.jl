struct BAOAB_Hidden{FP<:AbstractForce,TF<:AbstractFloat,TM} <: HiddenVelocityVerletIntegrator
    force::FP
    C::Matrix{TF}
    γ::Matrix{TF}
    M::TM
...
* Δt    - Time step
"""
function BAOAB_Hidden(force::FP, C::Array{TF}, A::Array{TF}, M::TM, Δt::TF, dim::Int, bc::Union{AbstractSpace,Nothing}=nothing) where {FP<:AbstractForce,TF<:AbstractFloat,TM}
    dim_tot = size(A)[1]
    friction = exp(-1 * Δt * A)
    C_sym = 0.5 .* (C .+ C')
    return BAOAB_Hidden(
...
mutable struct HiddenVelocityVerletState{TF<:AbstractFloat} <: AbstractMemoryHiddenState
    ...
    ξ::Vector{TF}
    ξ₂::Vector{TF}
    function HiddenVelocityVerletState(x₀::Vector{TF}, v₀::Vector{TF}, h₀::Vector{TF},f::Vector{TF}) where {TF<:AbstractFloat}
...
function UpdateState!(state::VelocityVerletState, integrator::BAOAB_Hidden; kwargs...)
    ...
    state.v̂_mid = integrator.c₂ .* state.v_mid  .+ integrator.c₂_vh * state.h .+ state.ξ[1:integrator.dim]
    state.h = integrator.c₂_hv * state.v_mid .+ integrator.c₂_hh * state.h .+ state.ξ[1+integrator.dim:integrator.dim_tot]
